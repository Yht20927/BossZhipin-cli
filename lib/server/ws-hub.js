...
class WebSocketHub {
  ...
  constructor(options) {
    ...
    /** @type {Map<string, { failures: number, timer: NodeJS.Timeout|null, warningSent: boolean }>} */
  ...
  _startHeartbeat(connId, ws) {
    const timer = setInterval(() => {
      ...
      // 每个 tick 计一次未收到 PONG 的失败；_handlePong 收到 PONG 时重置为 0。
      // 之前 failures 只被重置、从不自增，导致死连接永远不会被心跳检测关闭。
      hb.failures++;

      if (hb.failures >= this.heartbeatMaxFailures) {
      ...
      // 接近阈值时发送警告
      if (hb.failures === this.heartbeatMaxFailures - 1 && !hb.warningSent) {
        hb.warningSent = true;
        console.warn(`[ws-hub] Heartbeat warning for ${connId.slice(0, 8)}: ${hb.failures}/${this.heartbeatMaxFailures} failures`);
      }
    ...
    this._heartbeats.set(connId, { failures: 0, timer, warningSent: false });
  }
