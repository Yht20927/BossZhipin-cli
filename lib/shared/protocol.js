...
function validateCallRequest(body) {
  ...
  return {
    ...
    data: {
      ...
      // opType: 'write'（post/like/delete）触发服务端最小间隔节流；'read' 不触发。
      opType: body.opType === 'write' || body.opType === 'read' ? body.opType : 'read',
      // noThrottle: true 跳过节流（--fast 调试通道）；默认 false。
      noThrottle: body.noThrottle === true,
    },
  };
}
